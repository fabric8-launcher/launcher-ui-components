...
import { keyBy, get, uniqBy } from 'lodash';
...
function filterExample(query, catalog) {
    var missionById = keyBy(catalog.missions, 'id');
    var runtimeById = keyBy(catalog.runtimes, 'id');
    var result = [];
...
function filterExampleMission(query, catalog) {
    ...
    var lambda = function (name, obj) {
        return uniqBy(filterExamples(catalog.boosters, undefined, obj.id), function (b) { return b.runtime; }).map(
        // @ts-ignore
...
function checkRunsOnCluster(example, cluster) {
    ...
    var runsOn = get(example, 'metadata.app.launcher.runsOn');
...
function defaultLauncherClient(config) {
    var injector = ReflectiveInjector.resolveAndCreate([HttpService]);
...
export { defaultLauncherClient, mockLauncherClient, ExampleAppDescriptor, toRuntime, fillPropsValuesWithEnums, propsWithValuesMapper, filterExample, filterExampleMission, filterExampleRuntime, filter, filterExamples, checkNotNull };
